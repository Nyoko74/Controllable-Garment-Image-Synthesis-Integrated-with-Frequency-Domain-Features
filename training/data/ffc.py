...
import PIL
from tools.gen_feature_img import display_feature, display_frequency_feature
...
class FourierUnit(nn.Module):
    ...
    def forward(self, x):
        ...
        # visualization
        # display_frequency_feature(ffted, from_model="fft")
...
class WaveletUnit(nn.Module):
    def __init__(self, in_channels, out_channels, groups=1, spatial_scale_factor=None, spatial_scale_mode='bilinear',
        ...
        self.dwt = DWTForward2d(J=3, mode='zero', wave='db3')
        self.conv1 = torch.nn.Conv2d(in_channels=in_channels * 2 + (2 if spectral_pos_encoding else 0),
                                          out_channels=out_channels * 2,
                                          kernel_size=1, stride=1, padding=0, groups=self.groups, bias=False)
        self.conv2 = torch.nn.Conv2d(in_channels=in_channels * 2 + (2 if spectral_pos_encoding else 0),
                                          out_channels=out_channels * 2,
                                          kernel_size=1, stride=1, padding=0, groups=self.groups, bias=False)
        self.conv3 = torch.nn.Conv2d(in_channels=in_channels * 2 + (2 if spectral_pos_encoding else 0),
                                          out_channels=out_channels * 2,
                                          kernel_size=1, stride=1, padding=0, groups=self.groups, bias=False)
...
class SpectralTransform(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)
        output = self.fu(x)
        # display_feature(output, from_model="fu") # output fft feature
        if self.enable_lfu:
            n, c, h, w = x.shape
            split_no = 2
            split_s = h // split_no
            ...
            xs = torch.cat(torch.split(xs, split_s, dim=-1),
                           dim=1).contiguous()
            xs = self.lfu(xs)
            # display_feature(xs, from_model="lfu")  # output fft feature
