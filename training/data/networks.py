...
class EncoderNetwork(torch.nn.Module):
    ...
    def forward(self, img, c, **block_kwargs):
        ...
        B, _ = x.shape
        z = torch.randn((B, self.z_dim), requires_grad=False, dtype=x.dtype,
...
@persistence.persistent_class
class SketchEncoderNetwork(torch.nn.Module):
    def __init__(self,
                 c_dim,  # Conditioning label (C) dimensionality.
                 z_dim,  # Input latent (Z) dimensionality.
                 img_resolution,  # Input resolution.
...
class SynthesisNetwork(torch.nn.Module):
    ...
    def forward(self, x_global, feats, sketch_feats, ws, fname=None, **block_kwargs):

        img = None

        x, img = self.foreword(x_global, ws, feats, sketch_feats, img)
        ...
        for res in self.block_resolutions:
            ...
            x, img = block(x, feats, sketch_feats, img, (mod_vector0, mod_vector1, mod_vector_rgb), fname=fname, **block_kwargs)
        return img
...
class Generator(torch.nn.Module):
    def __init__(self,
        ...
        self.sketch_encoder = SketchEncoderNetwork(c_dim=c_dim, z_dim=z_dim, img_resolution=img_resolution,
                                      img_channels=img_channels, **encoder_kwargs)
        self.scft = SCFT(base=1024, use_fp16=False)
    ...
    def forward(self, img, c, fname=None, truncation_psi=1, truncation_cutoff=None, **synthesis_kwargs):
        ...
        x_global, z, feats = self.encoder(img_texture, c)
        x_sketch, sketch_feats = self.sketch_encoder(img_sketch, c)
        # x_global = x_global + x_sketch  # method1. add features
        x_global = self.scft(x_sketch, x_global)  # method2. self-attention
        ws = self.mapping(z, c, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff)
        img = self.synthesis(x_global, feats, sketch_feats, ws, fname=fname, **synthesis_kwargs)
