...
class GarmentDataset(Dataset):
    ...
    def _get_image(self, idx):
        ...
        img_fname = os.path.join(self.img_path, fname)
        mask_fname =  os.path.join(self.mask_path, fname)

        sketch = np.array(self._load_image(sketch_fname))
        img = np.array(self._load_image(img_fname))  # uint8
        mask = np.array(self._load_image(mask_fname))

        mask[mask > 235] = 255
        mask[mask <= 235] = 0
        mask = np.expand_dims(mask[:, :, 0], axis=0)

        texture = self._get_texture(img, mask)

        return img, texture, sketch, fname.split('/')[-1].replace(ext, '')

    def _get_texture(self, groundtruth, mask):
    ...
    def __getitem__(self, idx):
        img, texture, sketch, fnames = self._get_image(idx)  # modal, uint8 {0, 1}

        texture_img = np.zeros_like(img)
        texture = self._put_patch_center(texture, texture_img)
        ...
        return img, texture, sketch, fnames

class GarmentTestDataset(Dataset):
    def __init__(self,
                 img_path,  # Path to images.
                 resolution=None,  # Ensure specific resolution, None = highest available.
                 **super_kwargs,  # Additional arguments for the Dataset base class.
