...
def setup_snapshot_image_grid(training_set, random_seed=0):
    ...
    # Load data.
    images, masks, sketch, texture, labels = zip(*[training_set[i] for i in grid_indices])
    return (gw, gh), np.stack(images), np.stack(masks), np.stack(sketch), np.stack(texture), np.stack(labels)

#----------------------------------------------------------------------------

def save_image_grid(img, texture, sketch, inv_mask, pred_img, fname, drange, grid_size):
    lo, hi = (0, 255)
    ...
    f_img = np.concatenate((img, texture, sketch, inv_mask * 255, pred_img), axis=1)
...
def training_loop(
    ...
    kimg_per_tick           = 4,        # (4)Progress snapshot interval.
    image_snapshot_ticks    = 50,       # (50)How often to save image snapshots? None = disable.
    network_snapshot_ticks  = 50,       # (50)How often to save network snapshots? None = disable.
    resume_pkl              = None,     # Network pickle to resume training from.
    resume_kimg             = 0,        # What kimg to start log.
    ...
    ddp_modules = dict()
    for name, module in [('G_encoder', G.encoder),('G_sketch_encoder', G.sketch_encoder),('G_scft', G.scft),('G_mapping', G.mapping), ('G_synthesis', G.synthesis), ('D', D), (None, G_ema), ('augment_pipe', augment_pipe)]:
    ...
    if rank == 0:
        ...
        grid_size, images, masks, sketches, textures, labels = setup_snapshot_image_grid(training_set=training_set)
        ...
        grid_sketch = (torch.from_numpy(sketches).to(torch.float32) / 127.5 - 1).to(device)
        grid_texture = (torch.from_numpy(textures).to(torch.float32) / 127.5 - 1).to(device)

        grid_img = grid_img.split(batch_gpu)
        grid_mask = grid_mask.split(batch_gpu)
        grid_texture = grid_texture.split(batch_gpu)
        grid_sketch = grid_sketch.split(batch_gpu)

        grid_c = torch.from_numpy(labels).to(torch.float32).to(device).split(batch_gpu)

        pred_images = torch.cat([G_ema(img=torch.cat([sketch_img, texture_img], dim=1), c=c, noise_mode='const').cpu() for sketch_img, texture_img, c in zip(grid_sketch, grid_texture, grid_c)])
        save_image_grid(images, textures, sketches, masks, pred_images.detach().numpy(), os.path.join(run_dir, 'run_init'), drange=[-1,1], grid_size=grid_size)
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            ...
            phase_real_imgs, phase_masks, phase_sketchs, phase_textures,  phase_real_cs = next(training_set_iterator)
            phase_real_img = (phase_real_imgs.to(device).to(torch.float32) / 127.5 - 1)
            phase_inv_mask = (phase_masks.to(device).to(torch.float32) / 255)
            phase_sketch = (phase_sketchs.to(device).to(torch.float32) / 127.5 - 1)
            phase_texture = (phase_textures.to(device).to(torch.float32) / 127.5 - 1)



            phase_real_img = phase_real_img.split(batch_gpu)
            phase_inv_mask = phase_inv_mask.split(batch_gpu)
            phase_sketch =phase_sketch.split(batch_gpu)
            phase_texture = phase_texture.split(batch_gpu)
        ...
        for phase, phase_gen_c in zip(phases, all_gen_c):
            ...
            for round_idx, (real_img, mask, sketch, texture, real_c, gen_c) in enumerate(zip(phase_real_img, phase_inv_mask, phase_sketch, phase_texture, phase_real_c, phase_gen_c)):
                sync = (round_idx == batch_size // (batch_gpu * num_gpus) - 1)
                gain = phase.interval
                loss.accumulate_gradients(phase=phase.name, real_img=real_img, mask=mask, sketch_img=sketch, texture_img=texture, real_c=real_c, gen_c=gen_c, sync=sync, gain=gain)
        ...
        if (snapshot_data is not None) and metrics and (done or cur_tick % network_snapshot_ticks == 0) : # and cur_tick is not 0:
            ...
            save_gen(snapshot_data['G_ema'], rank, num_gpus, device, eval_img_data, resolution, label, 1)
            if rank == 0:
                eval_dataset = PrecomputedInpaintingResultsDataset(eval_img_data, f'fid_gens/')
                metrics = {
                    'fid': FIDScore()
                }
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            pred_images = torch.cat(
                [G_ema(img=torch.cat([sketch_img, texture_img], dim=1), c=c, noise_mode='const').cpu() for
                 sketch_img, texture_img, c in zip(grid_sketch, grid_texture, grid_c)])

            save_image_grid(images, textures, sketches, masks, pred_images.detach().numpy(), os.path.join(run_dir, f'run_{cur_nimg//1000:06d}'), drange=[-1,1], grid_size=grid_size)
        ...
        if stats_tfevents is not None:
            global_step = int(cur_nimg / 1e3) + resume_kimg
