...
class InpaintingEvaluator():
    ...
    def dist_evaluate(self, device, num_gpus, rank, model=None):
        ...
        dataloader = torch.utils.data.DataLoader(dataset=self.dataset, sampler=item_subset, batch_size=self.batch_size,
                                                 **data_loader_kwargs)
        ...
        for score_name, score in self.scores.items():
            ...
            with torch.no_grad():
                score.reset()
                for _, batch in tqdm.auto.tqdm(enumerate(dataloader, 0), total=len(dataloader),
                                               desc=score_name + f' on GPU: {device}'):
...
class GarmentEvaluator():
    def __init__(self, dataset, scores, area_grouping=True, bins=10, batch_size=32, device='cuda',
                 integral_func=None, integral_title=None, clamp_image_range=None,
                 use_mask=False, use_texture=False):
        """
        :param dataset: torch.utils.data.Dataset which contains images and masks
        :param scores: dict {score_name: EvaluatorScore object}
        :param area_grouping: in addition to the overall scores, allows to compute score for the groups of samples
    ...
    def evaluate(self, model=None):
        ...
        for score_name, score in tqdm.auto.tqdm(self.scores.items(), desc='scores'):
            ...
            with torch.no_grad():
                ...
                for batch in tqdm.auto.tqdm(self.dataloader, desc=score_name, leave=False):
                    batch = move_to_device(batch, self.device)
                    image_batch, sketch_batch = batch['image'], batch['sketch']
                    if self.clamp_image_range is not None:
                        image_batch = torch.clamp(image_batch,
                                                  min=self.clamp_image_range[0],
                                                  max=self.clamp_image_range[1])
                    if model is None:
                        ...
                        inpainted_batch = model(image_batch, sketch_batch)
                    score(inpainted_batch, image_batch, None)
                total_results, group_results = score.get_value(groups=groups)
        ...
        for score_name, score in self.scores.items():
            ...
            with torch.no_grad():
                ...
                for _, batch in tqdm.auto.tqdm(enumerate(dataloader, 0), total=len(dataloader),
                                               desc=score_name + f' on GPU: {device}'):
                    batch = move_to_device(batch, device)
                    if self.use_mask and self.use_texture:
                        image_batch, sketch_batch, mask_batch, texture_batch, texture_size_batch = batch['image'], batch['sketch'], batch['mask'], batch['texture'], batch['texture_size']
                    elif self.use_mask:
                        image_batch, sketch_batch, mask_batch = batch['image'], batch['sketch'],  batch['mask']
                    elif self.use_texture:
                        image_batch, sketch_batch, texture_batch, texture_size_batch = batch['image'], batch['sketch'], batch['texture'], batch['texture_size']
                    else:
                        image_batch, sketch_batch = batch['image'],batch['sketch']
                    ...
                    if model is None:
                        ...
                        inpainted_batch = model(image_batch, sketch_batch)
                    if score_name in ['c_fid', 'c_lpips'] and self.use_texture and self.use_mask:
                        batch_size = image_batch.size(0)
                        for i in range(batch_size):
                            start_index = (texture_batch[i].size(-1) - texture_size_batch[i]) // 2
                            end_index = start_index + texture_size_batch[i]
                            texture = texture_batch[i, :, start_index: end_index, start_index: end_index]
                            score(inpainted_batch[i], texture, mask_batch[i])
                    else:
                        score(inpainted_batch, image_batch)
                total_results, group_results = score.get_value(groups=groups)
