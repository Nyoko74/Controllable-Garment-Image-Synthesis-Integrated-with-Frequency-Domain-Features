...
class c_LPIPSScore(PairwiseScore):
    def __init__(self, model='net-lin', net='vgg', model_path=None, use_gpu=True):
        super().__init__()
        self.score = PerceptualLoss(model=model, net=net, model_path=model_path,
                                    use_gpu=use_gpu, spatial=False).eval()
    ...
    def forward(self, pred, target, mask=None, patch_num=8):
        assert pred.dim() == target.dim() == 3
        target_size = target.size(-1)
        pred_batch = randam_get_patch(pred, target_size, mask, patch_num, use_texture_mask=False)
        target_batch = target.clone()
        target_batch = torch.unsqueeze(target_batch,dim=0)
        target_batch = target_batch.repeat(patch_num, 1, 1, 1)
        batch_values = self.score(pred_batch, target_batch).flatten()
        self.individual_values = np.hstack([
            self.individual_values, batch_values.detach().cpu().numpy()
        ])
...
def randam_get_patch(image, patch_size, mask, patch_num, use_texture_mask=False):

    encode_size = patch_size
    ...
    if use_texture_mask:
        texture_mask = get_texture_mask(mask, encode_size)
    ...
    for _ in range(patch_num):
        for i in range(max_loop):
            ...
            if 0.98 <= torch.mean(mask_crop) <= 1:
                patch_image = image[:, x: x + encode_size, y:y + encode_size].unsqueeze(0)
        ...
        if not patch_getted:
            ...
            patch_image = image[:, clip_start_index: clip_end_index,
            ...
            if patch_images_block is None:
                ...
                patch_images_block = torch.cat([patch_images_block, patch_image], dim=0)
    return patch_images_block


def get_texture_mask(mask, encode_size):
...
class c_FIDScore(EvaluatorScore):
    def __init__(self, dims=2048, eps=1e-6):
        LOGGER.info("FIDscore init called")
        super().__init__()
        if getattr(FIDScore, '_MODEL', None) is None:
    ...
    def forward(self, pred, target, mask=None, patch_num=16):
        assert pred.dim() == target.dim() == 3
        target_size = target.size(-1)
        pred_batch = randam_get_patch(pred, target_size, mask, patch_num, use_texture_mask=False)
        target_batch = target.clone()
