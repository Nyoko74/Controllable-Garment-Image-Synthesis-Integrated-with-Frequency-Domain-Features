...
def run_gen(rank, num_gpus, temp_dir, G, img_data, network_pkl, resolution, label, truncation_psi, get_mask=True, get_original_texture=True):
    ...
    network_type = network_pkl.split('/')[-3]
    ...
    if rank == 0:
        eval_dataset = PrecomputedInpaintingResultsDataset(img_data, f'fid_gens/',
                                                           get_mask=get_mask,
        ...
        results.to_csv(f'fid_gens/{network_type}.csv', sep='\t', float_format='%.4f')
        print(results)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        if num_gpus == 1:
            run_gen(rank=0, num_gpus=num_gpus, temp_dir=temp_dir, G=G,
                        img_data=img_data, network_pkl=network_pkl, resolution=resolution, label=label, truncation_psi=truncation_psi)
        else:
